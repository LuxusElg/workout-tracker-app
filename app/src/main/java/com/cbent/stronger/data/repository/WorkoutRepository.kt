...
class WorkoutRepository(
	...
	fun getWorkoutFull(workoutId: Long): Flow<WorkoutFull> {
		return workoutDao.getWorkoutWithExercisesAndSets(workoutId)
			.map { workout ->
				workout.toDomain()
			}
	}
	...
	fun setsForExercise(exerciseId: Long): Flow<List<TrainingSet>> = workoutDao.getSetsForExercise(exerciseId)
	...
	suspend fun insertSet(trainingSet: TrainingSet) = workoutDao.insertSet(trainingSet)
	...
	suspend fun deleteSet(trainingSet: TrainingSet) = workoutDao.deleteSet(trainingSet)
}
